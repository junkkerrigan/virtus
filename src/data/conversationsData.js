export default {
  lyan: {
    active: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'lyan',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'lyan',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'lyan',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'lyan',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'viewed'
      }
    ],
    trash: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'lyan',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'lyan',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'lyan',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'lyan',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 4, 17, 32),
        sender: 'harry',
        status: 'trash'
      }
    ]
  },
  jolene: {
    active: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'new'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'new'
      },
      {
        text: 'It starts with words `Load up on guns, bring your friends, it`s fine to lose and to pretend`',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'new'
      }
    ],
    trash: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo.',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      },
      {
        text: 'It starts with words `Load up on guns, bring your friends, it`s fine to lose and to pretend`',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo.',
        date: new Date(2016, 4, 10, 16, 50),
        sender: 'jolene',
        status: 'trash'
      }
    ]
  },
  michelle: {
    active: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'new'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'new'
      },
      {
        text: 'It starts with words `Is this the real life? Is this just fantasy?`',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'new'
      }
    ],
    trash: [
      {
        text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusm.',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      },
      {
        text: 'It starts with words `Is this the real life? Is this just fantasy?`',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      },
      {
        text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusm.',
        date: new Date(2016, 4, 11, 17, 32),
        sender: 'michelle',
        status: 'trash'
      }
    ]
  },
  dominic: {
    active: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'new'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'new'
      },
      {
        text: 'It starts with words `I push my fingers into my EEEEYES`',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'new'
      }
    ],
    trash: [
      {
        text: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia.',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      },
      {
        text: 'It starts with words `I push my fingers into ny EEEEYES`',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      },
      {
        text: 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia.',
        date: new Date(2016, 4, 2, 0, 15),
        sender: 'dominic',
        status: 'trash'
      }
    ]
  },
  queen: {
    active: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'new'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'new'
      },
      {
        text: 'It starts with words `We passed upon the stairs, we spoken was and when`',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'new'
      }
    ],
    trash: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'trash'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'trash'
      },
      {
        text: 'It starts with words `We passed upon the stairs, we spoken was and when`',
        date: new Date(2016, 3, 29, 14, 2),
        sender: 'queen',
        status: 'trash'
      }
    ]
  },
  lanadelrey: {
    active: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'viewed'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'new'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'new'
      },
      {
        text: 'It starts with words `All I am is a man, I want the world in my hands`',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'new'
      }
    ],
    trash: [
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ulla pariatur.',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim veniam,ex ea commo!',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'harry',
        status: 'trash'
      },
      {
        text: 'Ut enim ad minim',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'trash'
      },
      {
        text: 'I created a song, would you like to read it?',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'trash'
      },
      {
        text: 'It starts with words `All I am is a man, I want the world in my hands`',
        date: new Date(2016, 3, 22, 15, 13),
        sender: 'lanadelrey',
        status: 'trash'
      }
    ]
  }
}